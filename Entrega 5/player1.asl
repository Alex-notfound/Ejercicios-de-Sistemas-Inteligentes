...
colocarPerder(X,EquipoAliado,EquipoEnemigo) :-
							tablero(X,Y,0)[source(percept)] & not unoEnRaya(X,EquipoAliado) & not bloquearCuatroEnRaya(X,EquipoAliado,EquipoEnemigo) & .print("TGUS")
							|tablero(X,Y,0)[source(percept)] & not unoEnRaya(X,EquipoAliado) & not bloquearTresEnRaya(X,EquipoAliado,EquipoEnemigo) & .print("asdada")
